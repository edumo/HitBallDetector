...
public class OpticalFlow_MovieFluid extends PApplet {
	...
	// velocity of the Fluid. The Movie Frames are the source for the
	// Fluid_density.
	//
	// This effect works best, when the movie background is not chaanging too
	// much,
	...
	private class MyFluidData implements DwFluid2D.FluidData {
		...
		public void addDensityTexture_cam(DwFluid2D fluid,
			...
			DwGLSLProgram shader = context
					.createShader("data/addDensityCam.frag");
			shader.begin();
		...
		// custom shader, to add temperature from a texture (PGraphics2D) to the
		// fluid.
		public void addTemperatureTexture(DwFluid2D fluid,
				DwOpticalFlow opticalflow) {
			context.begin();
			context.beginDraw(fluid.tex_temperature.dst);
			DwGLSLProgram shader = context
					.createShader("data/addTemperature.frag");
			shader.begin();
			shader.uniform2f("wh", fluid.fluid_w, fluid.fluid_h);
		...
		// custom shader, to add density from a texture (PGraphics2D) to the
		// fluid.
		public void addVelocityTexture(DwFluid2D fluid,
				DwOpticalFlow opticalflow) {
			context.begin();
			context.beginDraw(fluid.tex_velocity.dst);
			DwGLSLProgram shader = context
					.createShader("data/addVelocity.frag");
			shader.begin();
			shader.uniform2f("wh", fluid.fluid_w, fluid.fluid_h);
			shader.uniform1i("blend_mode", 2);
			shader.uniform1f("multiplier", 0.5f);
			shader.uniform1f("mix_value", 0.1f);
			shader.uniformTexture("tex_opticalflow",
					opticalflow.frameCurr.velocity);
	...
	public void setup() {
		...
		// to keep the file size small. please use one of your own videos
		// instead.
	...
	public void draw() {
		...
		if (movie.available()) {
			...
			if (APPLY_BILATERAL) {
				DwFilter.get(context).bilateral.apply(pg_movie, pg_temp, 5,
						0.10f, 4);
				swapCamBuffer();
			}
		...
		String txt_fps = String.format(getClass().getName()
				+ "   [size %d/%d]   [frame %d]   [fps %6.2f]", view_w, view_h,
	...
	class TimeLine {
		...
		public void draw(float mx, float my) {
			...
			if (inside(mx, my)) {
				...
				String hoover_str = String.format(Locale.ENGLISH, "%1.2f",
						hoover_pos);
	...
	public void createGUI() {
		...
		Group group_fluid = cp5.addGroup("fluid");
		{
			...
			cp5.addButton("reset").setGroup(group_fluid)
					.plugTo(this, "fluid_reset").setSize(80, 18)
					.setPosition(px, py);
			cp5.addButton("+").setGroup(group_fluid)
					.plugTo(this, "fluid_resizeUp").setSize(39, 18)
					.setPosition(px += 82, py);
			cp5.addButton("-").setGroup(group_fluid)
					.plugTo(this, "fluid_resizeDown").setSize(39, 18)
					.setPosition(px += 41, py);

			px = 10;

			cp5.addSlider("velocity").setGroup(group_fluid).setSize(sx, sy)
					.setPosition(px, py += (int) (oy * 1.5f)).setRange(0, 1)
			...
			cp5.addSlider("density").setGroup(group_fluid).setSize(sx, sy)
					.setPosition(px, py += oy).setRange(0, 1)
			...
			cp5.addSlider("iterations").setGroup(group_fluid).setSize(sx, sy)
					.setPosition(px, py += oy).setRange(0, 80)
			...
			cp5.addSlider("gridscale").setGroup(group_fluid).setSize(sx, sy)
					.setPosition(px, py += oy).setRange(0, 50)
					.setValue(fluid.param.gridscale)
					.plugTo(fluid.param, "gridscale");

			RadioButton rb_setFluid_DisplayMode = cp5
					.addRadio("fluid_displayMode").setGroup(group_fluid)
					.setSize(80, 18).setPosition(px, py += (int) (oy * 1.5f))
					.setSpacingColumn(2).setSpacingRow(2).setItemsPerRow(2)
					.addItem("Density", 0).addItem("Temperature", 1)
					.addItem("Pressure", 2).addItem("Velocity", 3)
			...
			cp5.addRadio("fluid_displayVelocityVectors").setGroup(group_fluid)
					.setSize(18, 18).setPosition(px, py += (int) (oy * 2.5f))
		...
		cp5.addAccordion("acc").setPosition(gui_x, gui_y).setWidth(gui_w)
				...
				.addItem(group_fluid).addItem(group_oflow)
				.addItem(group_display).open(0, 1, 2);
	}

	static public void main(String[] passedArgs) {
		String[] appletArgs = new String[] { "OpticalFlow_MovieFluid" };
		if (passedArgs != null) {
